...
check('recommendTables sorts tightest-fit first to avoid fragmentation', () => {
  const participants = [
    { tableId: 1 }, { tableId: 1 }, { tableId: 1 }, { tableId: 1 }, { tableId: 1 }, { tableId: 1 }, { tableId: 1 }, { tableId: 1 },
    // Mesa 1: 8/10 -> 2 left, Mesa 2: 0/10 -> 10 left
  ];
  const recs = recommendTables({ categoria: 'participante' }, tables, participants);
...
check('recommendTables excludes full tables and caps at 3 results', () => {
  const manyTables = Array.from({ length: 5 }, (_, i) => ({ id: 100 + i, name: `T${i}`, capacity: 10, reservedFor: null }));
